//  avl.cpp
//  notebook
//  Created by eren bezirgancı on 9.04.2022.
...
clone( AvlNode<Comparable> * t ) const
{
    if( t == NULL )
        ...
        return new
AvlNode<Comparable>( t->element, clone( t->left ), clone( t->right ) );
}
...
template <class Comparable>
void AvlTree<Comparable>::
remove(const Comparable & x, AvlNode<Comparable> * & t ) const
{
 if (t == NULL)
...
getBalance(AvlNode<Comparable> * t ) const
{
    if (t == NULL)
        return 0;
